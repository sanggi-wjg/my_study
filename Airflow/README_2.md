...
It is not intended to be used in production and it has a number of caveats - one of them being that the best way to recover from any problem is to clean it up and restart from the scratch.
```sh
...
airflow-webserver:
  ports:
    - 9988:9988
  healthcheck:
    test: ["CMD", "curl", "--fail", "http://localhost:9988/health"]
...
```
### Example 없이 docker-compose up 하기
```shell script
AIRFLOW__CORE__LOAD_EXAMPLES: 'false'
```

#### PostgreSQL 접속 정보는 변경하면 안된다. 
#### dags, logs, plugins 폴더는 공유 볼륨이므로 실행 이후 변경 하지 않는다.
...
secret_key = jk2XW/T4AQfLv35OczEm8w==
...
sensors_enabled = NamedHivePartitionSensorroot@demo-docker:/home/airflow
```
